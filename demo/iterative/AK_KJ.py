...
def it_part_J (J):
    return [[J.update(l_ind="tl"), Zero, Zero],
...
def repartK (K, *args, **kws):
    ret_dict = {}
    [K_l, k_m, K_r] = K
    ret_dict[K_l] = matrix([[K_l.update(l_ind="0")]])
    ret_dict[k_m] = matrix([[k_m.update(l_ind="1")]])
    ret_dict[K_r] = matrix([[K_r.update(l_ind="2", rank=1), K_r.update(l_ind="3")]])
    return ret_dict
...
def fuseK (K, *args, **kws):
    ret_dict = {}
    [K_l, k_m, K_r] = K
    ret_dict[K_l] = matrix([K_l.update(l_ind="0"), k_m.update (l_ind="1")])
    ret_dict[k_m] = matrix([K_r.update(l_ind="2", rank=1)])
    ret_dict[K_r] = matrix([K_r.update(l_ind="3")])
    return ret_dict
